...
class LigandFT:
    ...
    def __init__(self, mol, torTree, transPoints=None, covalentLigandAnchorAtoms=False, fixedRoot=False, neighborSearchCutoff=-1.0,atomsToExclude=None):
        ...
        if atomsToExclude:
            print('[LigandFT] Excluding %d receptor atoms from grid scoring.' % len(atomsToExclude))
            for idx in atomsToExclude:
                # The atom indices from runOneGA are 1-based PDBQT serials.
                # The C++ AtomSet is 0-based. So we must subtract 1.
                atom_index_0based = idx - 1
                
                # Sanity check to avoid errors with negative indices
                if atom_index_0based >= 0:
                    # This prevents Ligand B atoms from being scored against the grid
                    # potential generated by Ligand A atoms.
                    atomSetStatic.setScorableGrid(atom_index_0based, False)
                    
                    # This is also important. It prevents pairwise energy calculations
                    # if any part of Ligand A was made flexible.
                    atomSetStatic.setScorablePairwise(atom_index_0based, False)
