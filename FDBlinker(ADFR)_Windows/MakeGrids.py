...
def findBinary(filename):
    # This function is now modified to prioritize the known path.
    # The 'r' prefix handles backslashes correctly on Windows.
    known_path = r"C:\Program Files (x86)\ADFRsuite-1.0\bin\autogrid4.exe"
    if os.path.exists(known_path):
        return known_path

    # Fallback to original search logic if the hardcoded path doesn't exist.
    binary = None
    if (os.name != 'nt'):
...
class CalculateAD4Grids:
    ...
    def __init__(self, receptor, center, size, atypes, spacing=0.375,
        ...
        # --- FINAL MODIFICATION ---
        # The findBinary function will now provide the correct path.
        adBin = findBinary('autogrid4')
        assert(adBin is not None), "CRITICAL: 'autogrid4.exe' not found. Please check your ADFRsuite installation."
